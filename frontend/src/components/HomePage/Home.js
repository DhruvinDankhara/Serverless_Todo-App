...
export const Home = () => {
    ...
    const handleAddNewTask = async () => {
        ...
        if (newTask) {
            ...
            if (response) {
                const updateIndexTableData = tableData.map((t) => {
                    return { ...t, index: t.index + 1 };
                });

                setTableData([{ ...data, index: 1 }, ...updateIndexTableData]);
                setNewTask("");
            }
        }
    };
            ...
            render: (_, record) => (
                <div className={classes.action_buttons}>
                    ...
                    record.status !== "COMPLETED" ? (
                        <Button
                            onClick={() =>
                                handleUpdateStatus(record, "IN-PROGRESS")
                            }
                        >
                            In progress
                        </Button>
        ...
        <>
            ...
            >
                <h1 className={classes.title}>
                    Welcome, {localStorage.getItem("username")} to TODO
                    application
                </h1>
                <Divider />
                <div className={classes.container}>
                    ...
                    <Table
                        ...
                        scroll={true}
                        pagination={
                            tableData.length > 5 ? { pageSize: 5 } : false
                        }
                    />
                </div>
            </ConfigProvider>
